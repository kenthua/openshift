...
cd $TEMP_DIR
git clone https://github.com/kenthua/$SOURCE_APP.git
sed -i -e "s/gogs-ci.rhel-cdk.10.1.2.2.xip.io/$GOGS_ROUTE/" $OPENSHIFT_PWD/infrastructure/jenkins/source/Jenkinsfile
cd $SOURCE_APP
...
oc process -f infrastructure/sonarqube/sonarqube-postgres-template.yaml -v POSTGRESQL_USER=sonar,SONAR_POSTGRESQL_PASSWORD=password | oc create -f -n $OSE_CI_PROJECT - 
